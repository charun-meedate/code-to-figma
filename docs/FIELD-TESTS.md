...
**6 โปรเจกต์ + ฝั่งเขียน Figma จริง · 13 สิงหาคม 2026**
...
**รวม 30 ปัญหา** · selftest จาก 38 → 66 ข้อ · ทุกข้อที่เจอถูกล็อกด้วย assertion
...
**4 · `threshold = 24` ก็ผิดด้วย — และเจอ blind spot เชิงโครงสร้าง**

ค่าสุดท้ายที่สืบทอดมาโดยไม่เคยวัด · วัดกับภาพจริง 12 ใบ:

- **noise** ระดับ rasterizer: p99 = **29** · max = **68** → เส้น 24 ปล่อยหางของ noise ผ่าน
- **ความต่างจริง** (เลื่อนทั้งเฟรม 1px): median magnitude = **23** → **ต่ำกว่าเส้น 24**

สองกลุ่มทับกันเต็ม ๆ · เส้นระดับพิกเซลแยกไม่ได้ตั้งแต่แรก — ตัวที่แยกจริงคือการรวมเป็นแถว

กวาดตาราง threshold × row-threshold: **th48 / row5% ได้ noise 0.0 แถบ/ภาพ และยังตรวจเจอ 12/12**
(ของเดิม th24/row5% = 4.5 แถบ/ภาพ) → เปลี่ยนเป็น **48**

**แต่ระหว่างวัดเจอของที่สำคัญกว่าค่าคงที่:**

| ความผิดพลาดเชิงโครงสร้าง | peak row % | เห็นไหม |
|---|---|---|
| ขอบการ์ดกว้างขึ้น 8px | 36% | เห็น |
| ปุ่มกว้าง 200px เลื่อน 2px | 26% | เห็น |
| **ไอคอน 24px เลื่อน 4px** | **0.5%** | **ไม่เห็น** |

**ไอคอนถูกมองข้ามที่ทุก threshold รวมทั้งค่าเดิม** — ไม่ใช่ปัญหาการจูน แต่เป็นข้อจำกัด
ของวิธี: element กว้าง 24px บนแถว 390px ต่อให้เลื่อนก็กินแถวไม่ถึง 2%

→ **diff สะอาดไม่ได้แปลว่าไม่มีอะไรขยับ แปลว่าไม่มีอะไร*กว้าง ๆ* ขยับ**
...
**7 · ยิงกับดักที่เหลือครบ 10/10 — 3 ตัวสุดท้ายยังจริงทั้งหมด**

`saveVersionHistoryAsync` ยัง throw · `ARROW_EQUILATERAL` ยังใส่หัวลูกศร**สองปลาย**
(ยืนยันด้วย screenshot) · ซ่อนลูกของ auto-layout ยังทำให้ตัวล่างเลื่อนขึ้น (container 140→90)

**สรุปตารางทั้ง 10 แถว: ยังจริง 6 · เกินจริง 2 · เลิกเป็นกับดักแล้ว 2**

**8 · โหมด light/dark เป็นฟีเจอร์ที่ต้องจ่ายเงิน — และ skill ไม่เคยบอก**

`addMode` throw ว่า **`Limited to 1 modes only`** บนไฟล์ tier starter · เขียนสคริปต์เลี่ยงไม่ได้
เพราะเป็นข้อจำกัดของแพลน

**นี่บล็อกเรื่อง light/dark ทั้งเรื่องของ skill** — P1 เขียนว่า "คอลเลกชันสีหนึ่งชุด มี mode ต่อ theme",
intake ถามว่า "จะทำ theme ไหนบ้าง", profile มีช่อง `themes` · **ไม่มีที่ไหนบอกว่าต้องมีแพลนก่อน**

→ เพิ่ม **P0 gate 0b** พร้อม probe 6 บรรทัดที่ยิงถามก่อนสัญญาอะไร · เพิ่มลง discovery D7
(เช็ค tier จาก `whoami` ตอนถามเรื่อง Figma อยู่แล้ว) · และ `tokens.md` บอกทางเลือกเมื่อทำไม่ได้
— **ห้ามเงียบ ๆ ทำแค่ light แล้วปล่อยให้ของที่ส่งดูเหมือนมีสองธีม**

**9 · text style / effect style / `ignoreFigma` ผ่านหมด**

- text style 3/3 · effect style 2/2 อ่านกลับมาค่าตรง (float เพี้ยนแบบเดียวกับสี จัดการทางเดียวกัน)
- **ไม่ติดข้อจำกัดแพลนเหมือน mode** — ทำได้บน starter
- `ignoreFigma`: ไฟล์มี 34 variable ที่ programme ไม่ได้เป็นเจ้าของ → ไม่ waive **exit 1**
  (gate กั้นถูก) · waive แล้ว **exit 0** และรายงานเขียน `extra in Figma: 0 (34 waived)`
  — waive แบบเห็น ไม่ใช่แบบเงียบ

**10 · 2 บทเรียนเรื่องวิธีเขียน probe (พลาดเองทั้งคู่)**

- **cleanup ต้องอยู่ใน `finally`** — probe ที่ catch error ตัวเอง จะไม่ crash แปลว่าบรรทัด
  cleanup ที่อยู่*หลัง*จุด throw ไม่ถูกรัน → ทิ้งขยะไว้ในไฟล์ลูกค้า · `use_figma` atomic
  ช่วยตอนสคริปต์ throw เอง ไม่ช่วยตอนเราจับ error ไว้ (ยืนยัน: สคริปต์ที่ throw จริง
  ถูก roll back สะอาด แต่ตัวที่ catch ทิ้ง collection `_probe` ไว้)
- **เครื่องมือต้องเข้ากับข้ออ้าง** — ข้ออ้างเชิงภาพต้องตรวจด้วยภาพ · probe แรกของผมอ่าน
  property ที่ LINE node ไม่มี แล้วได้ error ที่**อ่านเหมือน "ตรวจไม่ได้" แต่จริง ๆ คือ "ใช้เครื่องมือผิด"**
  · `screenshot()` ครั้งเดียวจบ
...
- **T3 / T4 กับโปรเจกต์จริง** — วาดหน้าจอทั้งหน้าและเส้น flow · ยังไม่มีโปรเจกต์ไหนที่มี
  catalog ใช้งานได้ให้เป็นฐานเทียบ
- **P3 ทำได้แล้วในไฟล์ scratch** (component master + audit ผ่าน) แต่**ยังไม่เคยเทียบ
  master กับ render ของ component จริง** — ซึ่งคือหัวใจของ T2
...
- **โหมด 2 ธีมในไฟล์ที่แพลนรองรับ** — ยืนยันได้แค่ว่า starter ทำไม่ได้ ยังไม่ได้ยืนยันว่า
  แพลนที่รองรับแล้วทำงานถูก
- **แพลน Enterprise / REST API** สำหรับ push แบบ headless ใน CI — ยังไม่ยืนยัน
