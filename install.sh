...
for SKILL_NAME in $(available_skills); do
  ...
  # be a local edit someone has not pushed back to this repo yet. The two
  # Flutter skills in particular pre-date this repo and may exist as loose
  # copies in ~/.claude/skills or inside a project.
...
Next:
  Open Claude Code in the project and ask for a Figma design system built from
  the code — for example "build a Figma design system from this codebase".

The skill runs a discovery scan first, then asks two questions (build a
component catalog? how far do you want to go?) before it touches anything.
Nothing is created until you answer.
EOF
